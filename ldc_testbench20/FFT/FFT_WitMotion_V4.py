...
def y_periodogramSS(inputsignal,fsamp):
    N = len(inputsignal)
    N_notnan = np.count_nonzero(~np.isnan(inputsignal))
    hr = fsamp/N #frequency resolution
    t = np.arange(0,N*Ts,Ts)
...
def z_periodogramSS(inputsignal,fsamp):
    N = len(inputsignal)
    N_notnan = np.count_nonzero(~np.isnan(inputsignal))
    hr = fsamp/N #frequency resolution
    t = np.arange(0,N*Ts,Ts)
...
if __name__ == "__main__":
   ...
   # fs = 10*f1 #
    ...
    (fax_y, Aspectrum_y)    = y_periodogramSS(y, fs)
    (fax_z, Aspectrum_z)    = z_periodogramSS(z, fs)
...
#CREATE TIME PLOT FOR X with local peaks ploted
    ...
    amp_thresh_pos = x_max - 0.2
    amp_thresh_neg = (x_min * -1) - 0.2

    #mult sig by -1 to find negative peaks - used in min_peaks
    inv_np = -1 * x_np

    peaks, _ = find_peaks(x_np, height=amp_thresh_pos)
...
#CREAT FFT PLOT FOR X AXIS - ADD LOCAL PEAKS/MINS
