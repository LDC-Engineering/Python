...
"""
STREAM DATA COMPLETED - RAW VOLTAGE ARRAYS FOR EACH AXIS
"""


"""
PROCESS RAW V DATA INTO G DATA
ADXL326 ZERO and CALFACTOR APPLIED
"""

for i in range(0, len(x_array)):
    x_array[i] = (x_array[i]-CALZERO_X) * CALFACTOR_X

for i in range(0, len(z_array)):
    z_array[i] = (z_array[i]-CALZERO_Z) * CALFACTOR_Z
...
'''
AT THIS POINT 
LOCAL ARRAYS FILLED WITH PROCESSED ACCEL (G) DATA FOR EACH AXIS
'''

'''Print the average X acceleration across the entire sample'''
avg_x = sum(x_array)/len(x_array)
print ("Average Gx = " + str(avg_x))
...
'''
AT THIS POINT
We have a spreadsheet filled with timestamped acceleration data for each axis
'''

time.sleep(1)


'''READ DATA BACK INTO PYTHON- NOT SURE WHY I DO THIS'''
...
#NOTE- ONLY reading x-axis values into data1_array at this point
#will add additional axes in future
